...
export default class SpaceEngine {
	...
	render(){
        ...
        // add stars using a seeded random number generator
        const stars = getStars([bounds.left, bounds.top, bounds.right - bounds.left, bounds.bottom - bounds.top], 128);
        ...
        for (let regionX = startRegionX; regionX <= endRegionX; regionX++) {
            for (let regionY = startRegionY; regionY <= endRegionY; regionY++) {
                // if there is a region at this location, draw gray box
                // draw if region is selected
                const key = `${regionX}_${regionY}`;
                if (this.mapMaker.map[key]) {
                    const color = getSpaceColor(regionX * 8 * 32, regionY * 8 * 32)
        ...
        if (this.config.showPreview && this.clipboard?.regions) {
            ...
            for (const region of this.clipboard.regions) {
                const drawX = regionX + (region.x - this.clipboard.mouseRegion.x);
                const drawY = regionY + (region.y - this.clipboard.mouseRegion.y);

                // draw the region's tiles
                for (let row = 0; row < 8; row++) {
                    for (let col = 0; col < 8; col++) {
                        const tileType = region.tiles[row][col];
...
function getStars(rect, cellSize = 64) {
	const [x, y, w, h] = rect;
	const stars = [];
	...
	for (let cy = minY; cy <= maxY; cy++) {
		for (let cx = minX; cx <= maxX; cx++) {
			const random = hash2D(cx, cy);

			// Most cells contain no star.
			if (random > 0.15) continue;

			const offsetX = hash2D(cx, cy + 1000);
			const offsetY = hash2D(cx + 1000, cy);

			stars.push({
				x: cx * cellSize + offsetX * cellSize,
				y: cy * cellSize + offsetY * cellSize,
				size: 1 + hash2D(cx + 2000, cy) * 16
			});
		}
	}
...
function getRegionGradient(x, y) {
	const scale = 0.0008;
...
function getSpaceColor(x, y) {
	const noise = valueNoise(x * 0.00025, y * 0.00025);
	...
	const scaled = noise * (colors.length - 1);
	const index = Math.min(Math.floor(scaled), colors.length - 2);
	const amount = scaled - index;

	return lerpColor(
		colors[index].map(value => Math.min(value * brightness, 255)),
		colors[index + 1].map(value => Math.min(value * brightness, 255)),
		amount
	);
}